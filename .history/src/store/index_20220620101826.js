...
export default new Vuex.Store({
  ...
  mutations: {
    ...
    updateTask: function(state) {
      ...
      //変更しなかったら元々のを返す
      if(urgency === 0){
        urgency = state.Task[state.route.params.id-1].urgency
      }
   
      let impotance = state.newImpotance
      if(impotance === 0){
        impotance = state.Task[state.route.params.id-1].impotance
      }
      let day = state.newDay
      if(day === ""){
        day = state.Task[state.route.params.id-1].day
      }
      const updateTask={
        id: id,
        name: text,
      ...
      state.Task[state.route.params.id-1] = updateTask
      state.newTodo = "";
    },
    removeTask: function(state) {
      for (let i = state.Task.length - 1; i >= 0; i--) {
        if (Number(state.Task[i].id)) state.Task.splice(i, 1);
      }
    },
  },
